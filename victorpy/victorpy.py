...
class Page(object):
    def __init__(self, path, raw_content, site, page_type=PageType.CONTENT, template="page.html"):
        ...
        self.build_path = site.build_path
        ...
        if self.slug != "index":
            self.dest_path = os.path.join(self.build_path, os.path.dirname(self.relative_path), self.slug, "index.html")
        else:
            self.dest_path = os.path.join(self.build_path, os.path.dirname(self.relative_path), "index.html")
...
class Site(object):
    def __init__(self, base_dir: str):
        ...
        # Comme c'est un dictionnaire, il est complètement écrasé par une seule valeur paramétrée dans config.yaml...
        self.default_routes = {
            'tags.html': "/tags",
            'tag.html': "/tags/*",
        ...
        self.build_path = os.path.join(self.base_dir, self.params['build_dir_name'])
        self.templates_path = os.path.join(self.base_dir, self.params['templates_dir_name'])
        self.script_working_dir = os.path.dirname(os.path.realpath(__file__))
        self.default_templates_path = os.path.join(self.script_working_dir, "templates")

        self.template_env = jinja2.Environment(loader=jinja2.FileSystemLoader([self.templates_path, self.default_templates_path]), line_statement_prefix='#')
    ...
    def check_hooks(self):
        for hook_type, actions in self.params['hooks'].items():
            for action in actions:
                ...
                try:
                    load_module(path=os.path.join(self.base_dir, "actions", action+".py"), module_name=action)
                except Exception:
                    try:
                        load_module(path=os.path.join(self.script_working_dir, "actions", action+".py"), module_name=action)
                    except Exception as e:
        ...
        if 'after_render' in self.params['hooks']:
            for action in self.params['hooks']['after_render']:
                ...
                try:
                    ...
                    logging.error("Unable to launch hook {action}: {e}".format(action=action, e=e))
                finally:
    ...
    def _copy_static(self):
        ...
        dest = os.path.join(self.base_dir, self.params['build_dir_name'], self.params['static_dir_name'])
        try:
...
if __name__ == "__main__":
    ...
    logging.info("VictorPy v0.1")

    parser = argparse.ArgumentParser(description='A simple yet powerful static site generator')
    parser.add_argument("--verbosity", help="increase output verbosity")
    parser.add_argument("serve", nargs='?', default=None, help="Launch PySSM server on local directory")
    parser.add_argument("build", nargs='?', default=None, help="Generate site files in build directory")
    parser.add_argument("build-profile", nargs='?', default=None, help="Generate site files in build directory and generate profiling data")
    parser.add_argument("render-profile", nargs='?', default=None, help="Render site and generate profiling data")
    ...
    if action == 'build':
        ...
        site = Site(base_dir=os.getcwd())
        cProfile.run('site.build()', 'stats')
        stream = open('stats.txt', 'w');
        stats = pstats.Stats('stats', stream=stream).sort_stats('time')
        stats.print_stats()

    elif action == 'render-profile':
        import cProfile
        import pstats

        site = Site(base_dir=os.getcwd())
        cProfile.run('site.render()', 'stats')
        stream = open('stats.txt', 'w');
        stats = pstats.Stats('stats', stream=stream).sort_stats('time')
        stats.print_stats()
